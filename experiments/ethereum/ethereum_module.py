...
class EthereumModule(BlockchainModule):
    ...
    async def start_ethereum(self):
        ...
        if self.experiment.my_id == 1:
            cmd = "geth --datadir data --allow-insecure-unlock --metrics --pprof --pprofport %d --rpc --rpcport %d --rpcapi=\"eth,net,web3,personal,admin,debug,txpool\" --ethash.dagdir /home/pouwelse/ethash --port %d --networkid %d --nat extip:%s --mine --miner.threads=1 --maxpeers 500 --netrestrict 10.141.0.0/24 --txpool.globalslots=20000 --txpool.globalqueue=20000 > ethereum.out 2>&1" % (pprof_port, rpc_port, port, self.network_id, host)
        else:
            ...
            with open("/home/pouwelse/ethash/ethereum_node_1", "r") as bootstrap_node_file:
                bootstrap_enode = json.loads(bootstrap_node_file.read())["enode"]
            cmd = "geth --datadir data --allow-insecure-unlock --metrics --pprof --pprofport %d --rpc --rpcport %d --rpcapi=\"eth,net,web3,personal,admin,debug,txpool\" --ethash.dagdir /home/pouwelse/ethash --port %d --networkid %d --nat extip:%s --mine --miner.threads=1 --maxpeers 500 --netrestrict 10.141.0.0/24 --txpool.globalslots=20000 --txpool.globalqueue=20000 --bootnodes %s > ethereum.out 2>&1" % (pprof_port, rpc_port, port, self.network_id, host, bootstrap_enode)
    ...
    def start_monitor_tx_pool(self):
        ...
        def monitor_tx_pool():
            ...
            self.tx_pool_polls.append((time_elapsed, int(tx_pool_status["pending"], 16), int(tx_pool_status["queued"], 16)))
    ...
    def transfer(self):
        ...
        payload = {
            ...
            "params": [{
                ...
                "to": "0xafa3f8684e54059998bc3a7b0d2b0da075154d66",  # TODO: hard-coded
                "value": hex(100000)
            }, "password"],
            "jsonrpc": "2.0",
            "id": 0,
    ...
    def deploy_contract(self, contract_path, main_class_name):
        ...
        for str_client_id in self.all_vars.keys():
            ...
            self.experiment.send_message(client_id, b"contract_abi", hexlify(json.dumps(contract_interface['abi']).encode()))
