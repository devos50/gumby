...
class WavesModule(ExperimentModule):
    ...
    def __init__(self, experiment):
        ...
        self.matcher_account_info = self.generate_keys_from_seed(b"foo%d" % (self.picked_matcher_num - 1))
    ...
    def generate_keys_from_seed(self, seed_text):
        ...
        seedHash = hashChain(b'\0\0\0\0' + seed_text)
        ...
        unhashedAddress = chr(1).encode() + b"L" + hashChain(public_key)[0:20]
    ...
    def start_waves(self):
        ...
        wallet_text_seed = b"foo%d" % (self.experiment.scenario_runner._peernumber - 1)
        ...
        template_content = template_content.replace("<DEX_ACCOUNT>", self.account_info['address'])
        ...
        cmd = 'java -cp "/home/pouwelse/waves/waves-all-1.1.5.jar:/home/pouwelse/waves/lib/*" com.wavesplatform.Application waves-devnet.conf'
        self.waves_process = subprocess.Popen([cmd], shell=True, stdout=output_file)
